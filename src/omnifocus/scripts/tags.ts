import { serializeTagFn } from "../serializers.js";
...
export function buildListTagsScript(): string {
  ...
  var tags = document.flattenedTags.slice();
...
export function buildCreateTagScript(args: CreateTagArgs): string {
  ...
  var parent = null;
  if (args.parentTagId) {
    parent = document.flattenedTags.byId(args.parentTagId);
    if (!parent) throw new Error("Parent tag not found: " + args.parentTagId);
  } else if (args.parentTagName) {
    var matches = document.flattenedTags.filter(function(t) { return t.name === args.parentTagName; });
...
export function buildUpdateTagScript(args: UpdateTagArgs): string {
  ...
  var tag = document.flattenedTags.byId(args.id);
